...
const User = () => {
  ...
  const [user, setUser] = useState<{ data: { avatar: string; first_name: string; last_name: string; email: string } } | null>(null)
  ...
  return (
    <div>
      {user &&
      <div>
        <img src={user.data.avatar} alt={`${user.data.first_name} ${user.data.last_name}`} />
